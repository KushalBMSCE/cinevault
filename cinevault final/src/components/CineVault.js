...
import { movieService } from './supabase';
...
const CineVault = ({ user }) => {  // Added user prop here
  ...
  useEffect(() => {
    ...
    const searchMovies = async () => {
      ...
      setIsLoading(true);
      try {
        const data = await api.searchMovies(searchQuery, { signal: controller.signal });
        setMovies(data.results);
        setHasMore(false);
      } catch (error) {
  ...
  // Load favorites on initial render and when user changes
  useEffect(() => {
    const loadFavorites = async () => {
      if (!user) {
        setFavorites([]);
      ...
      try {
        ...
        // Extract movies from the join query and filter out any null values
  ...
  // In CineVault.js - Update loadFavorites function to handle RLS
  useEffect(() => {
    const loadFavorites = async () => {
      if (!user) {
        setFavorites([]);
